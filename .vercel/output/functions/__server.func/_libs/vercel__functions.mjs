    ...
    constructor({ endpoint, headers, onError, timeout = 500 }) {
      this.get = async (key) => {
        ...
        try {
          ...
          if (res.status === 200) {
            if (
              res.headers.get(import_index.HEADERS_VERCEL_CACHE_STATE) !==
              import_index.PkgCacheState.Fresh
            ) {
              res.body?.cancel?.();
              clearTimeout(timeoutId);
              return null;
...
var require_websocket = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  async function loadWebSocketServer() {
    try {
      return (await import("../_chunks/functions.mjs")).WebSocketServer;
    } catch {
      throw new Error(
  ...
  async function experimental_upgradeWebSocket(handler, options = {}) {
    ...
    const ws = await new Promise((resolve, reject) => {
      ...
      const onClose = () =>
        rejectUpgrade(
          /* @__PURE__ */ new Error("socket closed before the WebSocket upgrade completed"),
        );
